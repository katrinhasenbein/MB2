#########################################################################################################
### Script for a comparison of thresholding different band ratios (accuracy and sensitivity)
...
### This script analyses the accuracy and sensitvity of the thresholds of these ratios in the context of
### the glacier area and the accuracy.
###
...
#get information from name string of each image
...
for (i in ratios){
  ...
  #are there other criteria one can use to find the "best" threshold apart from the minimum between the
  #two humps of the histogram?
...
ggplot()+
  ...
  scale_color_manual(values = color_assignment, labels = names(results), name = "Band ratio")+
  ggtitle("ROC of the different ratios")
...
##save min and max thresholds in model_parameters data.frame
for (i in c(1:4)){
...
#plot the threshold ranges as factors of the standard deviation
threshold_range_sd <- model_parameters[,c(1, 20:23)]

#reshape the data in order to fit the requirements of ggplot
threshold_range_sd <- melt(threshold_range_sd, id.vars = c("ratio"))
threshold_range_sd$variable <-  c(rep("area", 8), rep("accuracy", 8))
threshold_range_sd$group <- paste(threshold_range_sd$ratio, threshold_range_sd$variable, sep = "_")
threshold_range_sd_area <- threshold_range_sd[threshold_range_sd$variable == "area",]
threshold_range_sd_accuracy <- threshold_range_sd[threshold_range_sd$variable == "accuracy",]

#area and accuracy in one plot
# ggplot(data = threshold_range_sd, aes(x = group, y = value))+
#   stat_summary(fun.ymin="min",fun.ymax="max",geom="errorbar")+
#   geom_hline(yintercept = 0)

#threshold ranges +/- 5% of true area
ggplot(data = threshold_range_sd_area, aes(x = ratio, y = value))+
  stat_summary(fun.ymin="min",fun.ymax="max",geom="errorbar")+
  geom_hline(yintercept = 0) + ylab("factor of standard deviation")+
  ggtitle("Threshold ranges +/- 5% of true area")

#threshold ranges >95% accuracy
ggplot(data = threshold_range_sd_accuracy, aes(x = ratio, y = value))+
  stat_summary(fun.ymin="min",fun.ymax="max",geom="errorbar")+ ylab("factor of standard deviation")+
  geom_hline(yintercept = 0) +ggtitle("Threshold ranges > 95% accuracy")
...
ggplot()+
  ...
  ggtitle("Comparison of different criteria to define the threshold")
...
#column names of the results variable (number of year + threshold, area, difference to optimal area, index)
...
for (i in 1:nrow(images)){
  ...
  #define band numbers according to Landsat sensor
  ...
  for (r in ratios){
    #calculate ratio and find threshold based on histogram
    ...
    #get min and max threshold: +/- 5% of "optimal" glacier area
...
#plot glacier area for all methods
...
#nir/swir and ndsi ratio shows some outliers (--> thresholding is not working properly) --> exclude unrealistic values
threshold_ranges_cleared <- threshold_ranges
for (i in 1: length(threshold_ranges_cleared)){
  threshold_ranges_cleared[[i]] <- threshold_ranges_cleared[[i]][threshold_ranges_cleared[[i]]$glacier_area < 500,]
}

ggplot() +
  geom_line(data = threshold_ranges_cleared[[1]], aes(x = year, y = glacier_area, colour = "A"))+
  geom_line(data = threshold_ranges_cleared[[2]], aes(x = year, y = glacier_area, colour = "B"))+
  geom_line(data = threshold_ranges_cleared[[3]], aes(x = year, y = glacier_area, colour = "C"))+
  geom_line(data = threshold_ranges_cleared[[4]], aes(x = year, y = glacier_area, colour = "D"))+
  scale_color_manual(values = color_assignment, labels = names(threshold_ranges_cleared),name = "Band ratio")
...
#boxplots with total threshold range as factor of the standard deviation
ggplot(data = sd_ranges, aes(y = sd_range, x = ratio))+
  #don't show outliers within boxplot
  geom_boxplot(outlier.size = 0, outlier.alpha = 0, outlier.color = NULL)+
  geom_jitter(width = 0.1)+
  ylab("total threshold range as factor of standard deviation")
