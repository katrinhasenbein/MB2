...
#' @description calculates the area under the curve and plots the result. Based on the results of \emph{sensitivity_analysis}
...
#' @param ratio_name name of the band ratio
...
area_under_curve <- function(fpr, tpr, ratio_name, plot.ROC){
  aoc <- 0.0
  ...
  if (plot.ROC == TRUE){
    ...
    plot(x[,1],x[,2], type = "l", main = ifelse(!missing(ratio_name), ratio_name, "ROC"))
    abline(a = 0, b = 1)
  }
  return(aoc)
}
