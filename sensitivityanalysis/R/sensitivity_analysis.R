...
sensitivity_analysis <- function(raster, val_raster, ratio_name, threshold_min, threshold_max, threshold, steps){
  ...
  if (missing(val_raster)){
    ...
    #if validation data is available, accuracy measurements can be calculated as well
  ...
  for (t in seq(threshold_min, threshold_max, step)){
    ...
    if (missing(val_raster)){
      ...
      #build confusion matrix to derive accuracy measurements
  ...
  #column with a index: threshold which is closest to optimal threshold: index = 0
